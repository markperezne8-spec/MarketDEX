...
9. `docs/governance/Approved_Architecture_Roadmap.md` is the approved implementation backlog.
10. The latest EC checkpoint records detailed in-progress evidence.
11. Draft PRs remain proposals until merged and do not override `main`.
...
1. Continue canonical domain and data architecture planning.
2. Map current repository code to the target modules using `KEEP`, `ADAPT`, `MIGRATE`, `RETIRE`, and `REVIEW` classifications.
3. Design one persistence, schema, and migration authority with backup, rollback, restart, and historical-fixture gates.
4. Define first command, query, event, repository, provider, and read-model contracts for Catalog, Inventory, Collection, Listings, Market Data, and Attention.
5. Preserve every material new idea or name in the idea register and EC checkpoint history.
6. Return to the Listing CI failure before marking any stacked PR ready or merging.
7. Re-run all gates on final heads and reconcile PRs #164, #165, and #166 in dependency order.
8. Build the desktop Collection Overview and Business/Collector mode toggle after the foundation stack is green and reconciled.
