...
| REQ-REP-001 | Analytics / Reports | immutable `ReportDefinition` and `ReportCatalog`; composition-owned `ReportQueryService`; Inventory Age request/provider/query boundaries; read-only `ReportsWorkspace`; canonical workspace registration | Build 701 definition, source-authority, request, query, composition, result-presentation, non-found, provenance, and visibility tests; dedicated Reports CI gate and Desktop Build composition coverage | In Progress |
