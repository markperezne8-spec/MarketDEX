...
**Baseline:** `main` after PR #124, with CAP-009C Build 500 delivery evidence pending PR #125 merge
...
| CAP-013 | Runtime database authority and migration | Desktop implementation authority | `launcher.py`; `core/runtime_database_migration.py`; schema version 20; runtime SQLite path | Core Tests CI gate; `test_runtime_database_authority.py` | Complete | Preserve single database authority |
...
**CAP-009 — Settlement allocation evidence and cross-check** is Complete for accepted workbook Builds 498-500 once PR #125 is merged and verified on `main`. CAP-009A supplies intake authority, CAP-009B supplies group-total and signed-remainder authority, and CAP-009C supplies fail-closed sale-level Settlement Attribution Readiness without executing settlement or mutating allocation evidence.

After CAP-009C merge verification, repository evidence identifies **CAP-010 — Allocation evidence revision and supersession / Build 502** as the next controlled authority boundary. CAP-011 Build 503 follows only after CAP-010.
