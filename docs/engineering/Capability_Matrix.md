...
**Baseline:** `main` after PR #126 / CAP-010A Build 502
...
**CAP-010 — Allocation evidence revision and supersession / Build 502** is Complete on `main` through merged PR #126. CAP-010A preserves immutable revision lineage, derives the exact workbook status vocabulary, detects multiple-current conflicts, fails closed outside a single current revision, blocks contradictory replay, and reconstructs authority from persisted evidence after restart.

Repository evidence identifies **CAP-011 — Allocation evidence lock and audit preservation / Build 503** as the next controlled authority boundary. CAP-011 must first reconcile the exact Build 503 workbook contract and must reuse the existing immutable audit and settlement allocation architecture rather than create a second lock architecture.
