...
class ExecutiveFeedbackLoopBlocked(RuntimeError): pass

STAGES=(
('EXECUTIVE_CYCLE_OBSERVATION','executive_control_cycles','executive_cycle_id','executive_cycle_result','EXECUTIVE_CONTROL_CYCLE_READY','executive_cycle_observations','executive_observation_id','executive_cycle_id','executive_observation_result','EXECUTIVE_CYCLE_OBSERVATION_READY'),
('EXECUTIVE_PERFORMANCE_RECONSTRUCTION','executive_cycle_observations','executive_observation_id','executive_observation_result','EXECUTIVE_CYCLE_OBSERVATION_READY','executive_performance_reconstructions','executive_performance_id','executive_observation_id','executive_performance_result','EXECUTIVE_PERFORMANCE_READY'),
('EXECUTIVE_VARIANCE_AUTHORITY','executive_performance_reconstructions','executive_performance_id','executive_performance_result','EXECUTIVE_PERFORMANCE_READY','executive_variances','executive_variance_id','executive_performance_id','executive_variance_result','EXECUTIVE_VARIANCE_READY'),
('EXECUTIVE_RESPONSE_AUTHORITY','executive_variances','executive_variance_id','executive_variance_result','EXECUTIVE_VARIANCE_READY','executive_responses','executive_response_id','executive_variance_id','executive_response_result','EXECUTIVE_RESPONSE_READY'),
('EXECUTIVE_FEEDBACK_LOOP_AUTHORITY','executive_responses','executive_response_id','executive_response_result','EXECUTIVE_RESPONSE_READY','executive_feedback_loops','executive_feedback_loop_id','executive_response_id','executive_feedback_loop_result','EXECUTIVE_FEEDBACK_LOOP_READY'))

class ExecutiveFeedbackLoopService(AuthoritativeService):
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveFeedbackLoopBlocked('Explicit complete executive feedback authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutiveFeedbackLoopBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutiveFeedbackLoopBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutiveFeedbackLoopBlocked(f'Second {stage} authority blocked')
   preserved=('sales','settlement_executions','order_closures','financial_finalizations','profit_recognitions','business_decisions','operating_commands','adaptive_operating_states','continuous_operating_loops','autonomous_business_cycles','strategic_control_loops','governed_strategic_cycles','enterprise_governance_loops','executive_control_cycles'); before=tuple(c.execute(f'SELECT COUNT(*) n FROM {x}').fetchone()['n'] for x in preserved)
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveFeedbackLoopBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
