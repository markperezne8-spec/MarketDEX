...
class ProductRegistryService:
    ...
    def _init(self):
      with self._c() as c:
       c.executescript("""
       CREATE TABLE IF NOT EXISTS event_identity(event_id TEXT PRIMARY KEY,request_id TEXT NOT NULL UNIQUE,event_type TEXT NOT NULL,payload_sha256 TEXT NOT NULL,created_at TEXT NOT NULL);
       CREATE TABLE IF NOT EXISTS replay_defense_history(request_id TEXT PRIMARY KEY,event_id TEXT NOT NULL,result_identity TEXT NOT NULL,committed_at TEXT NOT NULL);
       ...
       CREATE TABLE IF NOT EXISTS audit_events(audit_id TEXT PRIMARY KEY,event_id TEXT NOT NULL,authority_type TEXT NOT NULL,result TEXT NOT NULL,created_at TEXT NOT NULL);
    ...
    def register(self,product_type,canonical_name,set_name=None,card_number=None,variant_name=None,request_id=None):
      ...
      with self._c() as c:
       replay=c.execute('SELECT result_identity FROM replay_defense_history WHERE request_id=?',(request_id,)).fetchone()
       if replay: return replay['result_identity']
       ...
       c.execute('INSERT INTO event_identity VALUES(?,?,?,?,?)',(event_id,request_id,'PRODUCT_REGISTRATION',sha,ts))
       ...
       c.execute('INSERT INTO audit_events VALUES(?,?,?,?,?)',('AUD-'+uuid.uuid4().hex,event_id,'PRODUCT_REGISTRY','VERIFIED',ts))
       c.execute('INSERT INTO replay_defense_history VALUES(?,?,?,?)',(request_id,event_id,product_id,ts))
    ...
    def add_alias(self,product_id,alias_name,request_id):
      ...
      with self._c() as c:
       ...
       c.execute('INSERT INTO event_identity VALUES(?,?,?,?,?)',(event_id,request_id,'PRODUCT_ALIAS',sha,ts))
       ...
       c.execute('INSERT INTO audit_events VALUES(?,?,?,?,?)',('AUD-'+uuid.uuid4().hex,event_id,'PRODUCT_ALIAS','VERIFIED',ts))
       c.execute('INSERT INTO replay_defense_history VALUES(?,?,?,?)',(request_id,event_id,product_id,ts)); return product_id
    ...
    def verify(self,pid=None,duplicate='BLOCKED',alias_ok=None,collision='BLOCKED',replay=True):
      with self._c() as c:
       ...
       replay_row=c.execute('SELECT result_identity FROM replay_defense_history WHERE request_id=?',(ACCEPTANCE_REQUEST,)).fetchone()
       replay_ok=bool(replay and replay_row and replay_row['result_identity']==pid)
