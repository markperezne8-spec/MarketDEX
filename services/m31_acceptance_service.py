...
class M31AcceptanceService:
 ...
 def run(self):
  self._fixture()
  with self.database.connect() as c: already=c.execute("SELECT 1 FROM inventory_reconciliations WHERE reconciliation_id='M31-REC-1'").fetchone()
  ...
  with self.database.connect() as c:q=int(c.execute("SELECT quantity FROM inventory_authority WHERE asset_id='M31-ASSET'").fetchone()[0])
