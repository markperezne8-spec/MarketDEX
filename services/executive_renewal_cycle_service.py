...
class ExecutiveRenewalCycleBlocked(RuntimeError): pass

STAGES=(
('EXECUTIVE_EVOLUTION_OBSERVATION','executive_evolution_cycles','executive_evolution_cycle_id','executive_evolution_cycle_result','EXECUTIVE_EVOLUTION_CYCLE_READY','executive_evolution_observations','executive_evolution_observation_id','executive_evolution_cycle_id','executive_evolution_observation_result','EXECUTIVE_EVOLUTION_OBSERVATION_READY'),
('EXECUTIVE_RENEWAL_RECONSTRUCTION','executive_evolution_observations','executive_evolution_observation_id','executive_evolution_observation_result','EXECUTIVE_EVOLUTION_OBSERVATION_READY','executive_renewal_reconstructions','executive_renewal_id','executive_evolution_observation_id','executive_renewal_result','EXECUTIVE_RENEWAL_READY'),
('EXECUTIVE_RESILIENCE_AUTHORITY','executive_renewal_reconstructions','executive_renewal_id','executive_renewal_result','EXECUTIVE_RENEWAL_READY','executive_resilience_authorities','executive_resilience_id','executive_renewal_id','executive_resilience_result','EXECUTIVE_RESILIENCE_READY'),
('EXECUTIVE_REGENERATION_AUTHORITY','executive_resilience_authorities','executive_resilience_id','executive_resilience_result','EXECUTIVE_RESILIENCE_READY','executive_regenerations','executive_regeneration_id','executive_resilience_id','executive_regeneration_result','EXECUTIVE_REGENERATION_READY'),
('EXECUTIVE_RENEWAL_CYCLE_AUTHORITY','executive_regenerations','executive_regeneration_id','executive_regeneration_result','EXECUTIVE_REGENERATION_READY','executive_renewal_cycles','executive_renewal_cycle_id','executive_regeneration_id','executive_renewal_cycle_result','EXECUTIVE_RENEWAL_CYCLE_READY'))

class ExecutiveRenewalCycleService(AuthoritativeService):
 service_name='executive_renewal_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveRenewalCycleBlocked('Explicit complete executive renewal authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutiveRenewalCycleBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutiveRenewalCycleBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutiveRenewalCycleBlocked(f'Second {stage} authority blocked')
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveRenewalCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
