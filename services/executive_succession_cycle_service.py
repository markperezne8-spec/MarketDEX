...
class ExecutiveSuccessionCycleBlocked(RuntimeError): pass
...
('EXECUTIVE_SUCCESSION_RECONSTRUCTION','executive_continuity_observations','executive_continuity_observation_id','executive_continuity_observation_result','EXECUTIVE_CONTINUITY_OBSERVATION_READY','executive_succession_reconstructions','executive_succession_id','executive_continuity_observation_id','executive_succession_result','EXECUTIVE_SUCCESSION_READY'),
('EXECUTIVE_LEGACY_AUTHORITY','executive_succession_reconstructions','executive_succession_id','executive_succession_result','EXECUTIVE_SUCCESSION_READY','executive_legacy_authorities','executive_legacy_id','executive_succession_id','executive_legacy_result','EXECUTIVE_LEGACY_READY'),
('EXECUTIVE_STEWARDSHIP_AUTHORITY','executive_legacy_authorities','executive_legacy_id','executive_legacy_result','EXECUTIVE_LEGACY_READY','executive_stewardships','executive_stewardship_id','executive_legacy_id','executive_stewardship_result','EXECUTIVE_STEWARDSHIP_READY'),
('EXECUTIVE_SUCCESSION_CYCLE_AUTHORITY','executive_stewardships','executive_stewardship_id','executive_stewardship_result','EXECUTIVE_STEWARDSHIP_READY','executive_succession_cycles','executive_succession_cycle_id','executive_stewardship_id','executive_succession_cycle_result','EXECUTIVE_SUCCESSION_CYCLE_READY'))

class ExecutiveSuccessionCycleService(AuthoritativeService):
 service_name='executive_succession_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveSuccessionCycleBlocked('Explicit complete executive succession authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutiveSuccessionCycleBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutiveSuccessionCycleBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutiveSuccessionCycleBlocked(f'Second {stage} authority blocked')
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveSuccessionCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
