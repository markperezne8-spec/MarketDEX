...
class ExecutiveTransparencyCycleBlocked(RuntimeError): pass

STAGES=(
 ('EXECUTIVE_ACCOUNTABILITY_OBSERVATION','executive_accountability_cycles','executive_accountability_cycle_id','executive_accountability_cycle_result','EXECUTIVE_ACCOUNTABILITY_CYCLE_READY','executive_accountability_observations','executive_accountability_observation_id','executive_accountability_cycle_id','executive_accountability_observation_result','EXECUTIVE_ACCOUNTABILITY_OBSERVATION_READY'),
 ('EXECUTIVE_TRANSPARENCY_RECONSTRUCTION','executive_accountability_observations','executive_accountability_observation_id','executive_accountability_observation_result','EXECUTIVE_ACCOUNTABILITY_OBSERVATION_READY','executive_transparency_reconstructions','executive_transparency_id','executive_accountability_observation_id','executive_transparency_result','EXECUTIVE_TRANSPARENCY_READY'),
 ('EXECUTIVE_DISCLOSURE_AUTHORITY','executive_transparency_reconstructions','executive_transparency_id','executive_transparency_result','EXECUTIVE_TRANSPARENCY_READY','executive_disclosure_authorities','executive_disclosure_id','executive_transparency_id','executive_disclosure_result','EXECUTIVE_DISCLOSURE_READY'),
 ('EXECUTIVE_VISIBILITY_AUTHORITY','executive_disclosure_authorities','executive_disclosure_id','executive_disclosure_result','EXECUTIVE_DISCLOSURE_READY','executive_visibility_authorities','executive_visibility_id','executive_disclosure_id','executive_visibility_result','EXECUTIVE_VISIBILITY_READY'),
 ('EXECUTIVE_TRANSPARENCY_CYCLE_AUTHORITY','executive_visibility_authorities','executive_visibility_id','executive_visibility_result','EXECUTIVE_VISIBILITY_READY','executive_transparency_cycles','executive_transparency_cycle_id','executive_visibility_id','executive_transparency_cycle_result','EXECUTIVE_TRANSPARENCY_CYCLE_READY'))

class ExecutiveTransparencyCycleService(AuthoritativeService):
 service_name='executive_transparency_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveTransparencyCycleBlocked('Explicit complete executive transparency authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutiveTransparencyCycleBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutiveTransparencyCycleBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutiveTransparencyCycleBlocked(f'Second {stage} authority blocked')
   protected=('sales','settlement_executions','order_closures','financial_finalizations','profit_recognitions','business_decisions','operating_commands','adaptive_operating_states','continuous_operating_loops','autonomous_business_cycles','strategic_control_loops','governed_strategic_cycles','enterprise_governance_loops','executive_control_cycles','executive_feedback_loops','executive_learning_cycles','executive_evolution_cycles','executive_renewal_cycles','executive_continuity_cycles','executive_succession_cycles','executive_perpetuity_cycles','executive_institution_cycles','executive_covenant_cycles','executive_constitutional_cycles','executive_sovereignty_cycles','executive_legitimacy_cycles','executive_stewardship_cycles','executive_accountability_cycles')
   existing={x['name'] for x in c.execute("SELECT name FROM sqlite_master WHERE type='table'")}; preserved=tuple(x for x in protected if x in existing); before=tuple(c.execute(f'SELECT COUNT(*) n FROM {x}').fetchone()['n'] for x in preserved)
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveTransparencyCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
