...
class ExecutivePerpetuityCycleBlocked(RuntimeError): pass

STAGES=(
('EXECUTIVE_SUCCESSION_OBSERVATION','executive_succession_cycles','executive_succession_cycle_id','executive_succession_cycle_result','EXECUTIVE_SUCCESSION_CYCLE_READY','executive_succession_observations','executive_succession_observation_id','executive_succession_cycle_id','executive_succession_observation_result','EXECUTIVE_SUCCESSION_OBSERVATION_READY'),
('EXECUTIVE_PERPETUITY_RECONSTRUCTION','executive_succession_observations','executive_succession_observation_id','executive_succession_observation_result','EXECUTIVE_SUCCESSION_OBSERVATION_READY','executive_perpetuity_reconstructions','executive_perpetuity_id','executive_succession_observation_id','executive_perpetuity_result','EXECUTIVE_PERPETUITY_READY'),
('EXECUTIVE_ENDURANCE_AUTHORITY','executive_perpetuity_reconstructions','executive_perpetuity_id','executive_perpetuity_result','EXECUTIVE_PERPETUITY_READY','executive_endurance_authorities','executive_endurance_id','executive_perpetuity_id','executive_endurance_result','EXECUTIVE_ENDURANCE_READY'),
('EXECUTIVE_PRESERVATION_AUTHORITY','executive_endurance_authorities','executive_endurance_id','executive_endurance_result','EXECUTIVE_ENDURANCE_READY','executive_preservations','executive_preservation_id','executive_endurance_id','executive_preservation_result','EXECUTIVE_PRESERVATION_READY'),
('EXECUTIVE_PERPETUITY_CYCLE_AUTHORITY','executive_preservations','executive_preservation_id','executive_preservation_result','EXECUTIVE_PRESERVATION_READY','executive_perpetuity_cycles','executive_perpetuity_cycle_id','executive_preservation_id','executive_perpetuity_cycle_result','EXECUTIVE_PERPETUITY_CYCLE_READY'))

class ExecutivePerpetuityCycleService(AuthoritativeService):
 service_name='executive_perpetuity_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutivePerpetuityCycleBlocked('Explicit complete executive perpetuity authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutivePerpetuityCycleBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutivePerpetuityCycleBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutivePerpetuityCycleBlocked(f'Second {stage} authority blocked')
   preserved=('sales','settlement_executions','order_closures','financial_finalizations','profit_recognitions','business_decisions','operating_commands','adaptive_operating_states','continuous_operating_loops','autonomous_business_cycles','strategic_control_loops','governed_strategic_cycles','enterprise_governance_loops','executive_control_cycles','executive_feedback_loops','executive_learning_cycles','executive_evolution_cycles','executive_renewal_cycles','executive_continuity_cycles','executive_succession_cycles'); before=tuple(c.execute(f'SELECT COUNT(*) n FROM {x}').fetchone()['n'] for x in preserved)
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutivePerpetuityCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
