...
class ExecutiveContinuityCycleBlocked(RuntimeError): pass

STAGES=(
('EXECUTIVE_RENEWAL_OBSERVATION','executive_renewal_cycles','executive_renewal_cycle_id','executive_renewal_cycle_result','EXECUTIVE_RENEWAL_CYCLE_READY','executive_renewal_observations','executive_renewal_observation_id','executive_renewal_cycle_id','executive_renewal_observation_result','EXECUTIVE_RENEWAL_OBSERVATION_READY'),
('EXECUTIVE_CONTINUITY_RECONSTRUCTION','executive_renewal_observations','executive_renewal_observation_id','executive_renewal_observation_result','EXECUTIVE_RENEWAL_OBSERVATION_READY','executive_continuity_reconstructions','executive_continuity_id','executive_renewal_observation_id','executive_continuity_result','EXECUTIVE_CONTINUITY_READY'),
('EXECUTIVE_STABILITY_AUTHORITY','executive_continuity_reconstructions','executive_continuity_id','executive_continuity_result','EXECUTIVE_CONTINUITY_READY','executive_stability_authorities','executive_stability_id','executive_continuity_id','executive_stability_result','EXECUTIVE_STABILITY_READY'),
('EXECUTIVE_SUSTAINMENT_AUTHORITY','executive_stability_authorities','executive_stability_id','executive_stability_result','EXECUTIVE_STABILITY_READY','executive_sustainments','executive_sustainment_id','executive_stability_id','executive_sustainment_result','EXECUTIVE_SUSTAINMENT_READY'),
('EXECUTIVE_CONTINUITY_CYCLE_AUTHORITY','executive_sustainments','executive_sustainment_id','executive_sustainment_result','EXECUTIVE_SUSTAINMENT_READY','executive_continuity_cycles','executive_continuity_cycle_id','executive_sustainment_id','executive_continuity_cycle_result','EXECUTIVE_CONTINUITY_CYCLE_READY'))

class ExecutiveContinuityCycleService(AuthoritativeService):
 service_name='executive_continuity_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveContinuityCycleBlocked('Explicit complete executive continuity authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutiveContinuityCycleBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutiveContinuityCycleBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutiveContinuityCycleBlocked(f'Second {stage} authority blocked')
   preserved=('sales','settlement_executions','order_closures','financial_finalizations','profit_recognitions','business_decisions','operating_commands','adaptive_operating_states','continuous_operating_loops','autonomous_business_cycles','strategic_control_loops','governed_strategic_cycles','enterprise_governance_loops','executive_control_cycles','executive_feedback_loops','executive_learning_cycles','executive_evolution_cycles','executive_renewal_cycles'); before=tuple(c.execute(f'SELECT COUNT(*) n FROM {x}').fetchone()['n'] for x in preserved)
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveContinuityCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
