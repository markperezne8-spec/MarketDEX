...
class ExecutiveLegitimacyCycleBlocked(RuntimeError): pass

STAGES=(
('EXECUTIVE_SOVEREIGNTY_OBSERVATION','executive_sovereignty_cycles','executive_sovereignty_cycle_id','executive_sovereignty_cycle_result','EXECUTIVE_SOVEREIGNTY_CYCLE_READY','executive_sovereignty_observations','executive_sovereignty_observation_id','executive_sovereignty_cycle_id','executive_sovereignty_observation_result','EXECUTIVE_SOVEREIGNTY_OBSERVATION_READY'),
('EXECUTIVE_LEGITIMACY_RECONSTRUCTION','executive_sovereignty_observations','executive_sovereignty_observation_id','executive_sovereignty_observation_result','EXECUTIVE_SOVEREIGNTY_OBSERVATION_READY','executive_legitimacy_reconstructions','executive_legitimacy_id','executive_sovereignty_observation_id','executive_legitimacy_result','EXECUTIVE_LEGITIMACY_READY'),
('EXECUTIVE_CONSENT_AUTHORITY','executive_legitimacy_reconstructions','executive_legitimacy_id','executive_legitimacy_result','EXECUTIVE_LEGITIMACY_READY','executive_consent_authorities','executive_consent_id','executive_legitimacy_id','executive_consent_result','EXECUTIVE_CONSENT_READY'),
('EXECUTIVE_TRUST_AUTHORITY','executive_consent_authorities','executive_consent_id','executive_consent_result','EXECUTIVE_CONSENT_READY','executive_trust_authorities','executive_trust_id','executive_consent_id','executive_trust_result','EXECUTIVE_TRUST_READY'),
('EXECUTIVE_LEGITIMACY_CYCLE_AUTHORITY','executive_trust_authorities','executive_trust_id','executive_trust_result','EXECUTIVE_TRUST_READY','executive_legitimacy_cycles','executive_legitimacy_cycle_id','executive_trust_id','executive_legitimacy_cycle_result','EXECUTIVE_LEGITIMACY_CYCLE_READY'))

class ExecutiveLegitimacyCycleService(AuthoritativeService):
 service_name='executive_legitimacy_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveLegitimacyCycleBlocked('Explicit complete executive legitimacy authority required')
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveLegitimacyCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
