...
class ExecutiveCovenantCycleBlocked(RuntimeError): pass
...
('EXECUTIVE_COVENANT_RECONSTRUCTION','executive_institution_observations','executive_institution_observation_id','executive_institution_observation_result','EXECUTIVE_INSTITUTION_OBSERVATION_READY','executive_covenant_reconstructions','executive_covenant_id','executive_institution_observation_id','executive_covenant_result','EXECUTIVE_COVENANT_READY'),
('EXECUTIVE_MANDATE_AUTHORITY','executive_covenant_reconstructions','executive_covenant_id','executive_covenant_result','EXECUTIVE_COVENANT_READY','executive_mandate_authorities','executive_mandate_id','executive_covenant_id','executive_mandate_result','EXECUTIVE_MANDATE_READY'),
('EXECUTIVE_FIDUCIARY_AUTHORITY','executive_mandate_authorities','executive_mandate_id','executive_mandate_result','EXECUTIVE_MANDATE_READY','executive_fiduciaries','executive_fiduciary_id','executive_mandate_id','executive_fiduciary_result','EXECUTIVE_FIDUCIARY_READY'),
('EXECUTIVE_COVENANT_CYCLE_AUTHORITY','executive_fiduciaries','executive_fiduciary_id','executive_fiduciary_result','EXECUTIVE_FIDUCIARY_READY','executive_covenant_cycles','executive_covenant_cycle_id','executive_fiduciary_id','executive_covenant_cycle_result','EXECUTIVE_COVENANT_CYCLE_READY'))

class ExecutiveCovenantCycleService(AuthoritativeService):
 service_name='executive_covenant_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveCovenantCycleBlocked('Explicit complete executive covenant authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutiveCovenantCycleBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutiveCovenantCycleBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutiveCovenantCycleBlocked(f'Second {stage} authority blocked')
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveCovenantCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
