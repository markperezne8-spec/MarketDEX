...
class InventoryAppService(AuthoritativeService):
    ...
    def list_inventory(self, search_text='', asset_type='ALL', sort_key='NAME', sort_order='ASC'):
        ...
        with self.database.read_connection() as connection:
            rows = connection.execute("SELECT a.asset_id,a.asset_name,a.asset_type,i.quantity,i.total_cost_minor FROM assets a JOIN inventory_authority i ON i.asset_id=a.asset_id ORDER BY a.asset_name COLLATE NOCASE,a.asset_id").fetchall()
    ...
    def summarize_inventory(rows):
        rows = list(rows)
        return {'asset_count':len(rows),'total_units':sum(int(row['quantity']) for row in rows),'total_cost_minor':sum(int(row['total_cost_minor']) for row in rows)}
    ...
    def add_asset(self, *, asset_id, asset_name, asset_type, quantity, total_cost_minor, request_id):
        ...
        with self.database.transaction() as connection:
            ...
            connection.execute("INSERT INTO audit_events(event_id,authority_type,authority_id,verification_result,recorded_at) VALUES (?,?,?,?,?)", (event.event_id,'INVENTORY_ASSET',asset_id,'VERIFIED',event.committed_at)); self._verify_event(connection, event)
        return asset_id
    ...
    def bulk_adjust_assets(self, *, asset_ids, quantity_delta, cost_delta_minor, request_prefix):
        asset_ids = list(dict.fromkeys(str(asset_id).strip() for asset_id in asset_ids if str(asset_id).strip()))
        quantity_delta = int(quantity_delta); cost_delta_minor = int(cost_delta_minor); request_prefix = str(request_prefix or '').strip()
        ...
        for index, asset_id in enumerate(asset_ids):
            self.adjust_asset(asset_id=asset_id, quantity_delta=quantity_delta, cost_delta_minor=cost_delta_minor, request_id=f'{request_prefix}-asset-{index}')
            adjusted.append(asset_id)
        return adjusted