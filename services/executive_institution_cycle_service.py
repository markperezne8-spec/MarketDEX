...
class ExecutiveInstitutionCycleBlocked(RuntimeError): pass

STAGES=(
('EXECUTIVE_PERPETUITY_OBSERVATION','executive_perpetuity_cycles','executive_perpetuity_cycle_id','executive_perpetuity_cycle_result','EXECUTIVE_PERPETUITY_CYCLE_READY','executive_perpetuity_observations','executive_perpetuity_observation_id','executive_perpetuity_cycle_id','executive_perpetuity_observation_result','EXECUTIVE_PERPETUITY_OBSERVATION_READY'),
('EXECUTIVE_INSTITUTION_RECONSTRUCTION','executive_perpetuity_observations','executive_perpetuity_observation_id','executive_perpetuity_observation_result','EXECUTIVE_PERPETUITY_OBSERVATION_READY','executive_institution_reconstructions','executive_institution_id','executive_perpetuity_observation_id','executive_institution_result','EXECUTIVE_INSTITUTION_READY'),
('EXECUTIVE_CHARTER_AUTHORITY','executive_institution_reconstructions','executive_institution_id','executive_institution_result','EXECUTIVE_INSTITUTION_READY','executive_charter_authorities','executive_charter_id','executive_institution_id','executive_charter_result','EXECUTIVE_CHARTER_READY'),
('EXECUTIVE_DOCTRINE_AUTHORITY','executive_charter_authorities','executive_charter_id','executive_charter_result','EXECUTIVE_CHARTER_READY','executive_doctrines','executive_doctrine_id','executive_charter_id','executive_doctrine_result','EXECUTIVE_DOCTRINE_READY'),
('EXECUTIVE_INSTITUTION_CYCLE_AUTHORITY','executive_doctrines','executive_doctrine_id','executive_doctrine_result','EXECUTIVE_DOCTRINE_READY','executive_institution_cycles','executive_institution_cycle_id','executive_doctrine_id','executive_institution_cycle_result','EXECUTIVE_INSTITUTION_CYCLE_READY'))

class ExecutiveInstitutionCycleService(AuthoritativeService):
 service_name='executive_institution_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveInstitutionCycleBlocked('Explicit complete executive institution authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutiveInstitutionCycleBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutiveInstitutionCycleBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutiveInstitutionCycleBlocked(f'Second {stage} authority blocked')
   preserved=('sales','settlement_executions','order_closures','financial_finalizations','profit_recognitions','business_decisions','operating_commands','adaptive_operating_states','continuous_operating_loops','autonomous_business_cycles','strategic_control_loops','governed_strategic_cycles','enterprise_governance_loops','executive_control_cycles','executive_feedback_loops','executive_learning_cycles','executive_evolution_cycles','executive_renewal_cycles','executive_continuity_cycles','executive_succession_cycles','executive_perpetuity_cycles'); before=tuple(c.execute(f'SELECT COUNT(*) n FROM {x}').fetchone()['n'] for x in preserved)
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveInstitutionCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
