...
class AuthoritativeService:
 ...
 def _append_event_and_audit(self,c,event,action_name):
  ...
  if existing is not None:
   c.execute('INSERT OR IGNORE INTO replay_defense_history(request_id,original_event_id,attempted_event_type,payload_sha256,defense_result,recorded_at) VALUES (?,?,?,?,?,?)',(event.request_id,existing['event_id'],event.event_type,event.payload_sha256,'BLOCKED',event.committed_at))
   raise ReplayRejected(f'Request already committed: {event.request_id}')
