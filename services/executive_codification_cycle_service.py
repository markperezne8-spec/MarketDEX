...
class ExecutiveCodificationCycleBlocked(RuntimeError): pass

STAGES=(
('EXECUTIVE_INSTITUTION_OBSERVATION','executive_institution_cycles','executive_institution_cycle_id','executive_institution_cycle_result','EXECUTIVE_INSTITUTION_CYCLE_READY','executive_institution_observations','executive_institution_observation_id','executive_institution_cycle_id','executive_institution_observation_result','EXECUTIVE_INSTITUTION_OBSERVATION_READY'),
('EXECUTIVE_CODIFICATION_RECONSTRUCTION','executive_institution_observations','executive_institution_observation_id','executive_institution_observation_result','EXECUTIVE_INSTITUTION_OBSERVATION_READY','executive_codification_reconstructions','executive_codification_id','executive_institution_observation_id','executive_codification_result','EXECUTIVE_CODIFICATION_READY'),
('EXECUTIVE_STANDARD_AUTHORITY','executive_codification_reconstructions','executive_codification_id','executive_codification_result','EXECUTIVE_CODIFICATION_READY','executive_standard_authorities','executive_standard_id','executive_codification_id','executive_standard_result','EXECUTIVE_STANDARD_READY'),
('EXECUTIVE_CANON_AUTHORITY','executive_standard_authorities','executive_standard_id','executive_standard_result','EXECUTIVE_STANDARD_READY','executive_canons','executive_canon_id','executive_standard_id','executive_canon_result','EXECUTIVE_CANON_READY'),
('EXECUTIVE_CODIFICATION_CYCLE_AUTHORITY','executive_canons','executive_canon_id','executive_canon_result','EXECUTIVE_CANON_READY','executive_codification_cycles','executive_codification_cycle_id','executive_canon_id','executive_codification_cycle_result','EXECUTIVE_CODIFICATION_CYCLE_READY'))

class ExecutiveCodificationCycleService(AuthoritativeService):
 service_name='executive_codification_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveCodificationCycleBlocked('Explicit complete executive codification authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutiveCodificationCycleBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutiveCodificationCycleBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutiveCodificationCycleBlocked(f'Second {stage} authority blocked')
   preserved=('sales','settlement_executions','order_closures','financial_finalizations','profit_recognitions','business_decisions','operating_commands','adaptive_operating_states','continuous_operating_loops','autonomous_business_cycles','strategic_control_loops','governed_strategic_cycles','enterprise_governance_loops','executive_control_cycles','executive_feedback_loops','executive_learning_cycles','executive_evolution_cycles','executive_renewal_cycles','executive_continuity_cycles','executive_succession_cycles','executive_perpetuity_cycles','executive_institution_cycles'); before=tuple(c.execute(f'SELECT COUNT(*) n FROM {x}').fetchone()['n'] for x in preserved)
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveCodificationCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
