...
class ExecutiveDurabilityCycleBlocked(RuntimeError): pass

STAGES=(
('EXECUTIVE_CONTINUITY_OBSERVATION','executive_continuity_cycles','executive_continuity_cycle_id','executive_continuity_cycle_result','EXECUTIVE_CONTINUITY_CYCLE_READY','executive_continuity_observations','executive_continuity_observation_id','executive_continuity_cycle_id','executive_continuity_observation_result','EXECUTIVE_CONTINUITY_OBSERVATION_READY'),
('EXECUTIVE_DURABILITY_RECONSTRUCTION','executive_continuity_observations','executive_continuity_observation_id','executive_continuity_observation_result','EXECUTIVE_CONTINUITY_OBSERVATION_READY','executive_durability_reconstructions','executive_durability_id','executive_continuity_observation_id','executive_durability_result','EXECUTIVE_DURABILITY_READY'),
('EXECUTIVE_ENDURANCE_AUTHORITY','executive_durability_reconstructions','executive_durability_id','executive_durability_result','EXECUTIVE_DURABILITY_READY','executive_endurance_authorities','executive_endurance_id','executive_durability_id','executive_endurance_result','EXECUTIVE_ENDURANCE_READY'),
('EXECUTIVE_PERSISTENCE_AUTHORITY','executive_endurance_authorities','executive_endurance_id','executive_endurance_result','EXECUTIVE_ENDURANCE_READY','executive_persistences','executive_persistence_id','executive_endurance_id','executive_persistence_result','EXECUTIVE_PERSISTENCE_READY'),
('EXECUTIVE_DURABILITY_CYCLE_AUTHORITY','executive_persistences','executive_persistence_id','executive_persistence_result','EXECUTIVE_PERSISTENCE_READY','executive_durability_cycles','executive_durability_cycle_id','executive_persistence_id','executive_durability_cycle_result','EXECUTIVE_DURABILITY_CYCLE_READY'))

class ExecutiveDurabilityCycleService(AuthoritativeService):
 service_name='executive_durability_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveDurabilityCycleBlocked('Explicit complete executive durability authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutiveDurabilityCycleBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutiveDurabilityCycleBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutiveDurabilityCycleBlocked(f'Second {stage} authority blocked')
   preserved=('sales','settlement_executions','order_closures','financial_finalizations','profit_recognitions','business_decisions','operating_commands','adaptive_operating_states','continuous_operating_loops','autonomous_business_cycles','strategic_control_loops','governed_strategic_cycles','enterprise_governance_loops','executive_control_cycles','executive_feedback_loops','executive_learning_cycles','executive_evolution_cycles','executive_renewal_cycles','executive_continuity_cycles'); before=tuple(c.execute(f'SELECT COUNT(*) n FROM {x}').fetchone()['n'] for x in preserved)
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveDurabilityCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
