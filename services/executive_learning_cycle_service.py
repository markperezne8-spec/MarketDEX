...
class ExecutiveLearningCycleBlocked(RuntimeError): pass

STAGES=(
('EXECUTIVE_FEEDBACK_OBSERVATION','executive_feedback_loops','executive_feedback_loop_id','executive_feedback_loop_result','EXECUTIVE_FEEDBACK_LOOP_READY','executive_feedback_observations','executive_feedback_observation_id','executive_feedback_loop_id','executive_feedback_observation_result','EXECUTIVE_FEEDBACK_OBSERVATION_READY'),
('EXECUTIVE_LEARNING_RECONSTRUCTION','executive_feedback_observations','executive_feedback_observation_id','executive_feedback_observation_result','EXECUTIVE_FEEDBACK_OBSERVATION_READY','executive_learning_reconstructions','executive_learning_id','executive_feedback_observation_id','executive_learning_result','EXECUTIVE_LEARNING_READY'),
('EXECUTIVE_INSIGHT_AUTHORITY','executive_learning_reconstructions','executive_learning_id','executive_learning_result','EXECUTIVE_LEARNING_READY','executive_insights','executive_insight_id','executive_learning_id','executive_insight_result','EXECUTIVE_INSIGHT_READY'),
('EXECUTIVE_ADAPTATION_AUTHORITY','executive_insights','executive_insight_id','executive_insight_result','EXECUTIVE_INSIGHT_READY','executive_adaptations','executive_adaptation_id','executive_insight_id','executive_adaptation_result','EXECUTIVE_ADAPTATION_READY'),
('EXECUTIVE_LEARNING_CYCLE_AUTHORITY','executive_adaptations','executive_adaptation_id','executive_adaptation_result','EXECUTIVE_ADAPTATION_READY','executive_learning_cycles','executive_learning_cycle_id','executive_adaptation_id','executive_learning_cycle_result','EXECUTIVE_LEARNING_CYCLE_READY'))

class ExecutiveLearningCycleService(AuthoritativeService):
 service_name='executive_learning_cycle_service'
 ...
 def reconstruct(self,*,stage,authority_id,parent_id,request_id,intent):
  ...
  if not s or not all(str(v).strip() for v in(authority_id,parent_id,request_id)) or str(intent).upper()!=f'RECONSTRUCT_{stage}': raise ExecutiveLearningCycleBlocked('Explicit complete executive learning authority required')
  ...
  with self.database.read_connection() as c:
   ...
   if prior:
    ...
    raise ExecutiveLearningCycleBlocked(f'{stage} request identity mismatch')
  with self.database.transaction() as c:
   if not c.execute(f'SELECT 1 FROM {pt} WHERE {pid}=? AND {prc}=?',(parent_id,pr)).fetchone(): raise ExecutiveLearningCycleBlocked(f'Accepted {pr} authority required')
   if c.execute(f'SELECT 1 FROM {t} WHERE {pc}=?',(parent_id,)).fetchone(): raise ExecutiveLearningCycleBlocked(f'Second {stage} authority blocked')
   preserved=('sales','settlement_executions','order_closures','financial_finalizations','profit_recognitions','business_decisions','operating_commands','adaptive_operating_states','continuous_operating_loops','autonomous_business_cycles','strategic_control_loops','governed_strategic_cycles','enterprise_governance_loops','executive_control_cycles','executive_feedback_loops'); before=tuple(c.execute(f'SELECT COUNT(*) n FROM {x}').fetchone()['n'] for x in preserved)
 ...
 def get(self,stage,authority_id):
  ...
  with self.database.read_connection() as c:
   ...
   if not row or n!=1: raise ExecutiveLearningCycleBlocked(f'{stage} reconstruction failed')
   return dict(row)
 @staticmethod
