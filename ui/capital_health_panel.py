...
CAPITAL_HEALTH_VISUAL_CONTRACT = 'm1.16c-capital-health-display-states'
CAPITAL_HEALTH_GROUP_VISUAL_CONTRACT = 'm1.16c-capital-health-group-display-states'

CAPITAL_HEALTH_STATE_LABELS: dict[CapitalHealthState, tuple[str, StatusTone]] = {
...
def capital_health_state_badge_contract(
    ...
    try:
        return CAPITAL_HEALTH_STATE_LABELS[state]
...
class CapitalHealthPanel(MarketDEXDashboardPanel):
    ...
    def __init__(
        ...
        self.setProperty('visualContract', CAPITAL_HEALTH_VISUAL_CONTRACT)
        self.setProperty('capitalHealthState', self.view_model.state)
        self.setProperty(
            'capitalHealthGroupOrder',
            ','.join(CAPITAL_HEALTH_GROUP_ORDER),
        )
        state_label, state_tone = capital_health_state_badge_contract(
        ...
        self.state_badge.setProperty('capitalHealthState', self.view_model.state)
        self.state_badge.setProperty('capitalHealthDisplayLabel', state_label)
        ...
        for group in self.view_model.groups:
            ...
            group_widget.setProperty(
                'visualContract',
                CAPITAL_HEALTH_GROUP_VISUAL_CONTRACT,
            )
            group_state_label, group_state_tone = capital_health_state_badge_contract(
                group.state
            )
            ...
            group_state_badge.setProperty('capitalHealthState', group.state)
            group_state_badge.setProperty(
                'capitalHealthDisplayLabel',
                group_state_label,
            )
