...
class AddAssetDialog(QDialog):
    def __init__(self,parent=None):
        super().__init__(parent); self.setWindowTitle('Add Inventory Asset'); form=QFormLayout(self); self.name=QLineEdit(); self.asset_type=QComboBox(); self.asset_type.addItems(['SINGLE','SEALED','SLAB','ACCESSORY']); self.quantity=QSpinBox(); self.quantity.setRange(0,100000); self.quantity.setValue(1); self.cost=QDoubleSpinBox(); self.cost.setRange(0,1000000); self.cost.setDecimals(2); self.cost.setPrefix('
...
); self.product_name=QLineEdit(); self.set_name=QLineEdit(); self.item_condition=QComboBox(); self.item_condition.addItems(['','Near Mint','Lightly Played','Moderately Played','Heavily Played','Damaged','Sealed']); self.market_price=QDoubleSpinBox(); self.market_price.setRange(0,1000000); self.market_price.setDecimals(2); self.market_price.setPrefix('
