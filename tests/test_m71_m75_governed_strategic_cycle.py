...
def fixture(path):
 ...
 return GovernedStrategicCycleService(path)
...
def test_m71_m75_full_chain_exactly_once_replay_restart(tmp_path):
 path=tmp_path/'governed.sqlite3'; svc=fixture(path); first=run(svc); second=run(svc); third=run(GovernedStrategicCycleService(path)); assert first==second==third
...
def test_m71_m75_requires_parent_and_blocks_second(tmp_path):
 svc=fixture(tmp_path/'blocked.sqlite3')
 with pytest.raises(GovernedStrategicCycleBlocked): svc.reconstruct(stage='STRATEGIC_LOOP_EVALUATION',authority_id='BAD',parent_id='UNKNOWN',request_id='BAD-REQ',intent='RECONSTRUCT_STRATEGIC_LOOP_EVALUATION')
 run(svc)
 with pytest.raises(GovernedStrategicCycleBlocked): svc.reconstruct(stage='STRATEGIC_LOOP_EVALUATION',authority_id='SECOND',parent_id='CONTROL-LOOP-M70-001',request_id='SECOND-REQ',intent='RECONSTRUCT_STRATEGIC_LOOP_EVALUATION')

def test_m71_m75_history_append_only(tmp_path):
 svc=fixture(tmp_path/'append.sqlite3'); run(svc)
 with pytest.raises(sqlite3.IntegrityError):
  with svc.database.transaction() as c: c.execute('DELETE FROM governed_strategic_cycles_history')

def test_m71_m75_zero_upstream_mutation(tmp_path):
 svc=fixture(tmp_path/'preserve.sqlite3'); tables=('sales','settlement_executions','order_closures','financial_finalizations','profit_recognitions','business_decisions','operating_commands','adaptive_operating_states','continuous_operating_loops','autonomous_business_cycles','strategic_control_loops')
