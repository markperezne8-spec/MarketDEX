class InventoryRepository:
 ...
 def apply(self,c,*,asset_id,quantity_delta,cost_delta_minor,event_id,recorded_at):
  row=self.get(c,asset_id); q=int(row['quantity']) if row else 0; cost=int(row['total_cost_minor']) if row else 0; nq=q+quantity_delta; nc=cost+cost_delta_minor
  ...
  c.execute('INSERT INTO inventory_history(event_id,asset_id,quantity_delta,cost_delta_minor,resulting_quantity,resulting_total_cost_minor,recorded_at) VALUES (?,?,?,?,?,?,?)',(event_id,asset_id,quantity_delta,cost_delta_minor,nq,nc,recorded_at)); return nq,nc
